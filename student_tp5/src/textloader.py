...
def pad_collate_fn(samples: List[List[int]]):
    maxlen = max([len(sample) for sample in samples]) + 1

    padded_tensor = torch.cat((samples[0], torch.tensor([lettre2id["EOS"]])), dim=0)
    padded_tensor = F.pad(padded_tensor, pad=(0, maxlen - padded_tensor.size(0)))
    assert padded_tensor.size(0) == maxlen
    padded_tensor = padded_tensor.unsqueeze(0)

    for i in range(1, len(samples)):
        padded_sample = torch.cat((samples[i],torch.tensor([lettre2id["EOS"]])), dim=0)
        padded_sample = F.pad(padded_sample, pad=(0, maxlen - padded_sample.size(0)))
        padded_tensor = torch.cat((padded_tensor, padded_sample.unsqueeze(0)), dim=0)

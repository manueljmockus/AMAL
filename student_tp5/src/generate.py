...
def generate(rnn, emb, decoder, eos, start="", maxlen=200):
    ...
    #  TODO:  Implémentez la génération à partir du RNN, et d'une fonction decoder qui renvoie les logits (logarithme de probabilité 
    # à une constante près, i.e. ce qui vient avant le softmax) des différentes sorties possibles
    
    hidden_state = torch.zeros(32)
    ...
    seq = start
    current_symbol = string2code(seq[-1])
    while current_symbol != eos or len(seq) < maxlen:
        embedding = emb(current_symbol)
        current_symbol = model.decode(model.one_step(embedding, hstate).unsqueeze(0)).softmax(dim=1).argmax().item()
